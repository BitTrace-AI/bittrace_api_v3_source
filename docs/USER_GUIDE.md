...
Canonical source-lane freeze/export:
...
Verify a completed canonical campaign run root:
...
| --- | --- | --- |
| `configs/canonical_source_profile.yaml` | `bittrace campaign` | Copy for project work. Usually change `profile_name`, `data.raw_root`, and the project-owned selection, label, and split fields. Keep frozen stable-lane blocks fixed if you need the documented lane. |
| `configs/canonical_deployment_candidate.yaml` | `bittrace deployment-candidate` | Treat as the frozen supported-lane deployment-candidate search config. |
...
| --- | --- | --- |
| `bittrace campaign` | `runs/canonical_source_profile/<campaign_run_id>/` | `bt3.campaign_request.json`, `bt3.campaign_result.json` |
| `bittrace verify` | `runs/canonical_source_profile/<campaign_run_id>/07_parity_verification/` | `bt3.stage_request.json`, `bt3.verification_kit_manifest.json`, `bt3.golden_vector_manifest.json`, `bt3.parity_report.json` |
| `bittrace deployment-candidate` | `runs/canonical_deployment_candidate/<deployment_run_id>/` | `leanlean_deployment_candidate_summary.json`, `summary.md`, `persistence_prep/leanlean_persistence_tuning_prep.json` |
| `bittrace persistence` | `runs/canonical_deployment_candidate/<deployment_run_id>/persistence_tuning/<run_id>/` | `*_summary.json`, `*_profile.json`, `*_examples.json` |
...
You need a completed canonical campaign run root.
...
run_root = Path("runs/canonical_source_profile/<campaign_run_id>")
