"""Generic frozen S6 deploy/runtime inference helpers for canonical V3."""
...
def _coerce_waveform_record(record: WaveformDatasetRecord | Mapping[str, object]) -> WaveformDatasetRecord:
    ...
    if not isinstance(record, Mapping):
        raise ContractValidationError(
            "Frozen runtime inference expects a `WaveformDatasetRecord` or JSON-object payload."
        )
    if "context" in record:
        ...
        if not isinstance(context, Mapping):
            raise ContractValidationError("Frozen runtime record `context` must be a JSON object.")
...
def _packed_row_from_payload(payload: Mapping[str, object]) -> tuple[int, str]:
...
def _select_medoids(rows: Sequence[int], k_per_class: int) -> tuple[int, ...]:
    unique_rows = tuple(sorted(set(rows)))
    if not unique_rows:
        return ()
    k_eff = min(max(1, int(k_per_class)), len(unique_rows))
    if k_eff == 1:
        return (_select_single_medoid(unique_rows),)
    if comb(len(unique_rows), k_eff) <= _MAX_EXACT_MEDOID_COMBINATIONS:
        return _select_exact_medoids(unique_rows, k_eff)
    return _select_greedy_medoids(unique_rows, k_eff)
...
def _select_single_medoid(rows: Sequence[int]) -> int:
    return min(
        rows,
        key=lambda row: (
            sum(_hamming_distance(row, other) for other in rows),
            row,
        ),
    )


def _select_exact_medoids(rows: Sequence[int], k_eff: int) -> tuple[int, ...]:
    best_rows: tuple[int, ...] | None = None
    best_cost: int | None = None
    for candidate_rows in itertools.combinations(rows, k_eff):
        total_cost = sum(
            min(_hamming_distance(row, medoid) for medoid in candidate_rows)
            for row in rows
        )
    ...
    if best_rows is None:
        raise ContractValidationError("Frozen runtime failed to select exact medoids.")
    return best_rows


def _select_greedy_medoids(rows: Sequence[int], k_eff: int) -> tuple[int, ...]:
    selected = [_select_single_medoid(rows)]
    while len(selected) < k_eff:
        remaining = [row for row in rows if row not in selected]
        next_row = max(
            remaining,
            key=lambda row: (
                min(_hamming_distance(row, chosen) for chosen in selected),
                -row,
            ),
        )
...
class FrozenFrontendInference:
    """Deterministic deploy-style frontend output for one canonical record."""
...
@dataclass(frozen=True, slots=True)
class FrozenDeepInference:
    """Deterministic deploy-style deep output for one canonical record."""
...
@dataclass(frozen=True, slots=True)
class FrozenS6Runtime:
    """Loaded frozen S6 runtime surface backed by canonical exported artifacts."""
    ...
    def __post_init__(self) -> None:
        if self.deep_anchor_artifact.pass_fail != PassFail.PASS:
            raise ContractValidationError("Frozen runtime requires a passing Deep anchor artifact.")
        if self.frontend_export_reference.pass_fail != PassFail.PASS:
            raise ContractValidationError("Frozen runtime requires a passing frontend export reference.")
        if self.deep_anchor_artifact.deployed_winner is None:
            raise ContractValidationError("Frozen runtime requires an emitted deployed winner.")
        if self.freeze_export_manifest is not None:
            if self.freeze_export_manifest.pass_fail != PassFail.PASS:
                raise ContractValidationError("Frozen runtime requires a passing freeze/export manifest.")
            if self.freeze_export_manifest.anchor_artifact_ref != self.frontend_export_reference.deep_anchor_artifact_ref:
                raise ContractValidationError(
                    "Freeze/export manifest and frontend export reference must point to the same Deep anchor artifact."
                )

        deep_input_contract, deep_input_records = _load_deep_input_bundle(
        ...
        encoded_training_rows = {
            record.source_record_id: self.frontend_infer(record)
        ...
        for label in self._state_labels:
            ...
            medoids = _select_medoids(rows, selected_k)
    ...
    def frontend_infer(self, canonical_input: WaveformDatasetRecord | Mapping[str, object]) -> FrozenFrontendInference:
        record = _coerce_waveform_record(canonical_input)
        ...
        if frontend_input_id is None or frontend_fingerprint is None:
            raise ContractValidationError("Frozen runtime requires frontend lineage identifiers.")
        ...
        return FrozenFrontendInference(
    ...
    def deep_infer(
        ...
        *,
        ...
        frontend_output: FrozenFrontendInference | Mapping[str, object] | None = None,
    ) -> FrozenDeepInference:
        resolved_frontend = _coerce_frontend_output(
            self.frontend_infer(canonical_input) if frontend_output is None else frontend_output
        )
        class_distances = {
            label: min(
                (_hamming_distance(resolved_frontend.packed_row_int, prototype) for prototype in prototypes),
                default=None,
            )
        ...
        if not available_distances:
            payload = {
                ...
                "decision": "reject",
                "deploy_path": _deploy_path(self.freeze_export_manifest),
                "frontend_fingerprint": self.frontend_export_reference.frontend_lineage.frontend_fingerprint,
                "frontend_input_id": self.frontend_export_reference.frontend_lineage.frontend_input_id,
            ...
            return FrozenDeepInference(payload=payload, predicted_class=None, reject=True)
        ...
        return FrozenDeepInference(payload=payload, predicted_class=winning_class, reject=False)

    def end_to_end_infer(self, canonical_input: WaveformDatasetRecord | Mapping[str, object]) -> FrozenDeepInference:
        return self.deep_infer(canonical_input=canonical_input)


def load_frozen_s6_runtime(
    *,
    ...
    """Load the generic frozen S6 runtime surface from emitted export artifacts."""
    ...
    if deep_anchor_artifact_ref is None or frontend_export_reference_ref is None:
        raise ContractValidationError(
            "Frozen runtime loading requires the emitted Deep anchor and frontend export references."
        )
    ...
    if frontend_export_reference.deep_anchor_artifact_ref != deep_anchor_artifact_ref:
        raise ContractValidationError(
            "Frozen runtime artifact lineage mismatch: frontend export reference points to a different Deep anchor."
        )

    return FrozenS6Runtime(
...
def _optional_string(value: object) -> str | None:
    ...
    if not isinstance(value, str):
        raise ContractValidationError("Frozen runtime contract fields must preserve string values.")
    return value


def _load_deep_input_bundle(
    ...
    *,
    ...
    contract_payload = _load_json_mapping(
        ...
        field_name="FrozenRuntime.deep_input_contract",
    )
    handoff_payload = _load_json_mapping(
        handoff_manifest_path,
        field_name="FrozenRuntime.deep_input_handoff",
    )
    ...
    if contract_payload.get("handoff_manifest_path") != expected_handoff_path:
        raise ContractValidationError(
            "Frozen runtime deep-input contract lineage does not match the emitted handoff manifest."
        )
    if compute_json_sha256(contract_payload) != expected_bundle_fingerprint:
        raise ContractValidationError(
            "Frozen runtime deep-input contract fingerprint does not match the emitted frontend lineage."
        )
    if contract_payload.get("handoff_manifest_sha256") != compute_json_sha256(handoff_payload):
        raise ContractValidationError(
            "Frozen runtime deep-input handoff digest does not match the emitted contract lineage."
        )
    ...
    if not isinstance(raw_records, Sequence) or isinstance(raw_records, (str, bytes, bytearray)):
        raise ContractValidationError("Frozen runtime deep-input handoff must include a `records` sequence.")
    records = tuple(_coerce_waveform_record(record) for record in raw_records)
    if not records:
        raise ContractValidationError("Frozen runtime deep-input handoff must contain at least one record.")
    return contract_payload, records


def _deploy_path(manifest: FreezeExportManifest | None) -> str:
    if manifest is None or manifest.deploy_runtime is None:
        return "pure_symbolic"
    return manifest.deploy_runtime.deploy_path


def _coerce_frontend_output(
    output: FrozenFrontendInference | Mapping[str, object],
) -> FrozenFrontendInference:
    if isinstance(output, FrozenFrontendInference):
        return output
    if not isinstance(output, Mapping):
        raise ContractValidationError(
            "Frozen deep inference expects a `FrozenFrontendInference` or JSON-object payload."
        )
    ...
    if not isinstance(packed_row_int, int) or isinstance(packed_row_int, bool):
        raise ContractValidationError("Frozen frontend output must include integer `packed_row_int`.")
    if not isinstance(packed_row_sha256, str) or packed_row_sha256 == "":
        raise ContractValidationError("Frozen frontend output must include string `encoding_sha256`.")
    return FrozenFrontendInference(
        payload=dict(output),
